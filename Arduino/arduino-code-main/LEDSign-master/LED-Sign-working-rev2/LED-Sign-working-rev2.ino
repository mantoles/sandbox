 ...
 * A bug will leave the left most column on if the final character has set
 ...
 -----------------------------------------------------------------------------*/
//this version woerks.  Need to modify input based on status of i/o pins.
...
// Example: 7 1m 60-LED strips (similar to BetaBrite 80x7 display).  The following works
//#define LEDSTRIPS      1
...
#define ledBrightness  3

// Specify which font to use. (These examples are for the TVout fonts.)
//const unsigned char *font = font4x6; // works
//const unsigned char *font = font8x8ext;
const unsigned char *font = font6x8; // works

// Set these definitions to match the font data, or, if using the TVout
...
// beginning of while loop
  while(1)
  {
    strip.show(); // starts with a clean screen
    Serial.print("Enter message: ");
    msgLen = lineInput(message, maxMessageLength);
        ...
        offset = 0;
        {
          ...
          for (row=0; row<ROWS && row<fontHeight ; row++)
          {
            ...
            for (col=0; col<ledPerRow; col++)
            {
              ...
              {
                // 1 = set a pixel.
                strip.setPixelColor((row*ledPerRow)+colOffset,
                  strip.Color(127,0,0));
              ...
              {
                // 0 = unset a pixel.
                strip.setPixelColor((row*ledPerRow)+colOffset, 0);
            ...
            } // end of for (col=0; col<ledPerRow; col++)
...
#if defined(DEBUG)
          for (uint8_t i=0; i<ledPerRow; i++) Serial.print(F("-"));
          Serial.println();
#endif
          delay(scrollSpeed);
